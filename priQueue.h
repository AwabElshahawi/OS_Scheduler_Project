...
void priQueueInsert(priQueue *q, PCB *newProcess)
{
    ...
    while (cur != NULL)
    {
        ...
        if (newProcess->priority == cur->priority)
        {
            if (newProcess->arrival_time < cur->arrival_time)
                break;

            if (newProcess->arrival_time == cur->arrival_time && newProcess->id < cur->id)
                break;
        }
...
void printPriQueue(priQueue *q)
{
    ...
    while (cur != NULL)
    {
        printf("[id=%d pr=%d arr=%d] ", cur->id, cur->priority, cur->arrival_time);
        cur = cur->next;
    }
    printf("\n");
}
